...
#include<math.h>
#include "omp.h"	//To calculate time using omp_get_wtime()
...
void calc_force(struct Node** node, struct Quad* Quadtree, int num)		//To calculate force on a system of particles
{
	...
	/*
		Here we used omp.h just to measure the time for computation
		omp_get_wtime() is used to measure the time.
	*/
